...
model {
    ...
    target += beta_lpdf(alpha | 1, 1); // prior for alpha (we could also try uniform_lpdf(alpha,0,1) uniform prior)
    target += normal_lpdf(logTau | 0, 1); // prior for logtau, gaussian prior with mean 0 and sd 1 we can set normal prior becauseparam is transformed
