...
## Stan model 
```{r}
file <- file.path("clean_files/model_RL.stan")
...
trials <- 100  # Feel free to change this number to suit your needs
...
# Plot the histograms
hist(prior_draws_df$alpha_prior, 
  main = "Histogram of alpha_prior",
  xlab = "alpha_prior", col = "skyblue")

hist(prior_draws_df$tau_prior,
  main = "Histogram of tau_prior",
  xlab = "tau_prior", col = "salmon")
```

```{r}
# Flat priors model
mod_flat <- cmdstan_model("clean_files/stan_models_ppc/model_RL_flat_priors.stan", cpp_options = list(stan_threads = TRUE), stanc_options = list("O1"), pedantic = TRUE)

# Weak priors model
mod_weak <- cmdstan_model("clean_files/stan_models_ppc/model_RL_weak_priors.stan", cpp_options = list(stan_threads = TRUE), stanc_options = list("O1"), pedantic = TRUE)

# Informative priors model
mod_informed <- cmdstan_model("clean_files/stan_models_ppc/model_RL_informative_priors.stan", cpp_options = list(stan_threads = TRUE), stanc_options = list("O1"), pedantic = TRUE)
...
# Plot using ggplot2
library(ggplot2)
ggplot(combined_df, aes(x = mean_choice_prediction, fill = prior)) +
  geom_histogram(bins = 30, alpha = 0.6, position = "identity") +
  scale_fill_manual(values=c("Flat"="blue", "Weak"="orange", "Informed"="green")) +
  labs(title = "Summary of Prior Predictive Checks - Mean Choice Prediction",
...
results <- lapply(combination_labels, function(label) {
...
```{r}
# No changes to the Stan model are necessary for prior predictive checks. 
file2 <- file.path("clean_files/model_RL_prior_pred_check.stan")
mod2 <- cmdstan_model(file2, cpp_options = list(stan_threads = TRUE), stanc_options = list("O1"), pedantic = TRUE)

perform_prior_checks <- function(mod, trials) {
  data <- list(
    trials = trials,
    choice = rep(0, trials), # Dummy data, may not be used
    feedback = rep(0, trials), # Dummy data, may not be used
    prior_predictive_check = 1 # Indicating a prior predictive check
  )
  prior_samples <- mod$sample(
    data = data,
    fixed_param = TRUE,
    iter_sampling = 1000,
    refresh = 100,
    threads_per_chain = 2  # Add this line
  )
...
# Assuming 'mod' is your compiled Stan model
prior_checks <- perform_prior_checks(mod2, trials = 120)
```
```{r}
# Load the packages
library(cmdstanr)
...
# Assuming 'prior_checks' contains your Stan model output
# Extract draws for v1 and v2
v_values <- as_draws_df(prior_checks$draws(c("v1", "v2")))



v_long <- v_long %>%
  group_by(variable) %>%
  mutate(trial = row_number())  # Assigns a unique trial number within each variable group

ggplot(v_long, aes(x = trial, y = value, color = variable)) +
  geom_line(alpha = 0.1, show.legend = TRUE) +  # Adjust 'alpha' to your preference
  labs(title = "Prior Predictive Checks for v1 and v2", x = "Trial", y = "Value") +
  theme_minimal() +
  scale_color_manual(values = c("v1" = "blue", "v2" = "red"))  # Optional: Assign colors
  
```


```{r}
#MULTIPLE GAMES (NOT USED)s
First simulate 50 games of each parameter combination
```{r}
combinations <- expand.grid(alpha = c(0.1, 0.09), tau = c(0.1, 5))
trials <- 120
games <- 50
...
for(g in 1:games) {
  for(i in 1:nrow(combinations)) {
    ...
    results <- tibble('game' = rep(g, trials),
                      'trial' = 1:trials,
                      'RL_choice' = rep(NA, trials),
                      'WSLS_choice' = rep(NA, trials),
                      'v1' = rep(NA, trials),
...
```{r}
write_csv(all_results, "data/simdata_multiplevalues_multiplegames.csv")
```
