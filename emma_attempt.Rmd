...
for (game in 1:num_games) {
  ...
  RL_choices <- rep(NA, num_trials) 
  WSLS_choices <- rep(NA, num_trials)
  ...
  game_data <- data.frame(
    game = rep(game, num_trials),
    trial = seq(num_trials),
    RL_choices = RL_choices,
    WSLS_choices = WSLS_choices,
    RL_feedback = RL_feedback
  )
  df <- rbind(df, game_data)
}

# show results dataframe in a seperate window
results_df <- data.frame(Game = integer(), Trial = integer(), RL_Choice = integer(), WSLS_Choice = integer(), RL_Feedback = integer(), WSLS_Feedback = integer())
...
# Analyze the results (to be updated!!! 
